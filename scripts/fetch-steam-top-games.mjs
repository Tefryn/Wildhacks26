...
const OUTPUT_FILE             = "steam_top_games.json";
...
const fetchWithRetry = async (url, attempt = 1) => {
  try {
    ...
    // Try to parse JSON, but catch errors if response is not valid JSON
    try {
      return await res.json();
    } catch (jsonErr) {
      const text = await res.text();
      console.warn(`  Invalid JSON response from ${url}: ${text.substring(0, 100)}`);
      if (attempt < RETRY_ATTEMPTS) return fetchWithRetry(url, attempt + 1);
      return null;
    }
  } catch (err) {
