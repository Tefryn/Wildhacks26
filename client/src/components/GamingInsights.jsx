...
export default function GamingInsights({ timeline, eras }) {
  ...
  const averageCompletion = (
    games.length > 0
      ? (games.reduce((sum, g) => sum + g.achievementCompletionRate, 0) /
          games.length) *
        100
      : 0
  ).toFixed(1);

  const totalHours = Number(timeline.totalHours) || 0;
  const dayEquivalent = (totalHours / 24).toFixed(1);
  const avgHoursPerGame =
    games.length > 0 ? (totalHours / games.length).toFixed(1) : "0.0";
    ...
    <div className="gaming-insights">
        ...
        <div className="badges-grid">
          {perfectGames.length > 0 && (
            <div className="badge gold">
              ...
              <span className="badge-label">
                {perfectGames.length} Masterpieces
              </span>
          ...
          {masterpieces.length > 0 && (
            <div className="badge silver">
              <span className="badge-emoji">⭐</span>
              <span className="badge-label">
                {masterpieces.length} High Masters
              </span>
          ...
          {totalHours > 1000 && (
            <div className="badge platinum">
              ...
              <span className="badge-desc">
                {dayEquivalent}+ days of gameplay
              </span>
            </div>
          )}
        </div>
      ...
      <div className="fun-facts">
        ...
        <div className="facts-content">
          <p>
            🎮 You've invested <strong>{totalHours.toFixed(0)} hours</strong>{" "}
            into gaming this year—that's {dayEquivalent} days of pure play time!
          </p>
          <p>
            🎯 You have{" "}
            <strong>
              {perfectGames.length} games with 100% achievement completion
            </strong>
            , showing your mastery and dedication!
          </p>
          <p>
            📊 Your average completion rate is{" "}
            <strong>{averageCompletion}%</strong>, which is{" "}
            {parseFloat(averageCompletion) > 50
              ? "impressive!"
              : "respectable!"}
          </p>
          <p>
            🏆 As a <strong>{profileType}</strong>, you balance breadth and
            depth beautifully—{games.length} games with serious commitment to
            each!
          </p>
        </div>
      </div>
